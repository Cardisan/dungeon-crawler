...
function action(type){
	...
	if (type === 'deleteBranch'){ //In case of all options used for certain field removes blacklisted numbers, goes back to previous branching and adds current branching to blacklist
		lastMove().coords
	}
	if (type === 'unstuck'){ //In case of deadlock, goes back to previous branching and adds current branching to blacklist
		let keepGoing = true;
		while (keepGoing){
			if (matrix.blacklist[lastMove().coords[1] - 1][lastMove().coords[0] - 1].length === 0){ //for simple moves, just cancel them
				action('cancelOne');
			}
...
function saveGame(){
	save = [matrix, traversingTree];
}
function loadGame(){
	matrix = save[0];
	traversingTree = save[1];
}
...
function possibleNumbers(coords){ //accepts coordinates for a field and returns numbers which could be used in that field
	...
	for (let i = 0; i < localBlacklist.length; i++){
		console.log('possibleNumbers(): On this field there are numbers blacklisted!');
		if (pool.has(localBlacklist[i])){
			console.log(`First number blacklisted is ${localBlacklist[i]} and it is amongst the options, so it will be deleted!`);
...
function setup(){
	...
	sampleDataHard2 = [[0, 0, 0, 0, 0, 0, 0, 0, 8], [3, 0, 0, 0, 0, 0, 5, 0, 0], [0, 0, 4, 3, 0, 0, 0, 9, 1], [0, 0, 1, 0, 4, 6, 7, 5, 0], [0, 4, 9, 0, 0, 0, 0, 1, 0],
	...
	matrix = new Matrices(sampleDataHard2);
	matrix.updateOptions();
	checkEasiest();
}
function draw(){
